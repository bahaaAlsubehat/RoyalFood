    ...
    public class MenuManagementImp : IMenuManagement
    {
            ...
            try
            {
                ...
                foreach ( var x in items ) 
                {
                        ...
                        itemgetDTOs.Add(new ItemGetDTO
                        {
                            ...
                            myingitget = result.Where(y=>y.itemid ==x.ItemId).DistinctBy(t=>t.ingredientId).ToList(),
                            myimagesitem = result1.Where(y => y.itemid == x.ItemId).DistinctBy(t=>t.imgitmid).ToList()
            ...
            try
            {
                ...
                foreach (var m in allmeals)
                {
                    mealget.Add(new MealGet
                    {
                        ...
                        Mymealitem = result.Where(x=>x.mealid == m.MealId).DistinctBy(y=>y.itemid).ToList(),
                        myimagesmeal = result1.Where(x => x.mealid == m.MealId).DistinctBy(y => y.imageid).Distinct().ToList()
                    });
        ...
        public async Task<List<MealGet>> SortungMeal(int? id, string? namear, string? nameing, string? descar, string? descing, float? price)
        {
            try
            {
                ...
                if (id != null)
                {
                    meals = await _context.Meals.Where(x => x.MealId == id).ToListAsync();
                    var itemmeal = await _context.ItemMeals.ToListAsync();
                    var items = await _context.Items.ToListAsync();
                    var ingredients = await _context.Ingredients.ToListAsync();
                    var ingrditem = await _context.IngredientItems.ToListAsync();
                    var result = (from ing in ingredients
                                 join ingit in ingrditem on ing.IngredientId equals ingit.IngredientId
                                 join it in items on ingit.ItemId equals it.ItemId where it.ItemId == ingit.ItemId
                                 join mlit in itemmeal on it.ItemId equals mlit.ItemId where it.ItemId == mlit.ItemId
                    ...
                    foreach (var m in meals)
                    {
                        if(m.MealId == id)
                        {
                            mealget.Add(new MealGet
                            {
                                ...
                                lastmodificationdate = m.LastModificationDate.ToString(),
                                lastuseridmodified = m.LastModifiedUserId.ToString(),
                                Mymealitem = result.Where(x=>x.mealid == id).DistinctBy(y=>y.itemid).ToList(),
                                myimagesmeal = result1.Where(x=>x.mealid == m.MealId).DistinctBy(y=>y.imageid).ToList()
                            });
                        }
                    }
